...
class Populations:
    ...
    def random(n_populations: int, pop_size: int, n_inputs: int, n_outputs: int,
        ...
        return Populations(
            dnas=dnas, n_inputs=n_inputs, n_outputs=n_outputs,
            n_hidden=n_hidden, primitive_functions=primitive_functions,
            primitive_arities=primitive_arities,
            descending_fitness=descending_fitness, n_alleles=n_alleles,
            n_primitives=n_primitives, max_arity=max_arity, device=device)
    ...
    def __init__(self, dnas: torch.Tensor, n_inputs: int, n_outputs: int,
        ...
        self._n_primitives = len(primitive_functions) if n_primitives is None \
                             else n_primitives
        self._max_arity = max(primitive_arities) if max_arity is None \
                          else max_arity
        if isinstance(primitive_arities, list):
            self._primitive_arities = torch.tensor(primitive_arities,
                                                   device=device)
        else:
    ...
    def configuration(self) -> 'Populations.Configuration':
        return \
            Populations.Configuration(
                n_inputs=self._n_inputs, n_outputs=self._n_outputs,
    ...
    def validate(self, raise_: bool = True) -> bool:
        ...
        if not self._validate_number(
                len(self._primitive_functions), expected=self._n_primitives,
                name='len(primitive_functions)', raise_=raise_):
            return False
        if not self._validate_number(
                self._primitive_arities.size(0), expected=self._n_primitives,
                name='primitive_arities.size(0)', raise_=raise_):
            return False

        if not self._validate_number(
                self._max_arity, max(self._primitive_arities),
                name='max_arity', raise_=raise_):
            return False
        ...
        if not self._validate_number(
              dna_size, expected=n_nodes*(self._max_arity+1),
              name='DNA size', raise_=raise_):
            return False
        ...
        if not self._validate_tensor(
              self._n_alleles, expected=expected_n_alleles,
              name='n_alleles', raise_=raise_):
            return False

        if not torch.all(self._dnas >= 0) \
               and torch.all(self._dnas < self._n_alleles):
            if raise_:
