...
class TestPhenotype(unittest.TestCase):
    ...
    def test_crazy_dna(self):
        ...
        input = torch.linspace(-1., 1., 50).expand(1, -1)
        ...
        dnas = torch.tensor([[[0, 0, 0,
                              1, 0, 0, # z1 = x0 * x0 = x0^2
                              1, 1, 1, # z2 = z1 * z1 = x0^4
                              1, 2, 2, # z3 = z2 * z2 = z2^2 = x0^8
                              2, 2, 1,
                              1, 3, 3, # z5 = z3 * z3 = z3^2 = x0^16
                              3, 2, 5, # z6 = z2 / z5 = x0^4 / x0^16 = x0^(-12)
                              3, 4, 2,
                              2, 4, 0,
                              2, 4, 8,
                              2, 2, 2,
...
class TestPopulations(unittest.TestCase):
    ...
    def test_regression_tensor_sanity(self) -> None:
        ...
        input = torch.linspace(-1., 1., 50, device=device).expand(1, -1)
        true_output = target(input)
        ...
        for i in range(n_steps):
            ...
            W = 1./W
            self.assertFalse(torch.any(W.isnan() | W.isinf()),
                             f'Degenerate weights at step {i}')
            ...
            self.assertFalse(torch.any(offspring(input).isnan()),
                             f'NaN offspring output at step {i}')

            offspring.fitnesses = loss(offspring(input), true_output)

            self.assertFalse(torch.any(offspring.fitnesses.isnan()),
    ...
    def test_koza3_parity(self) -> None:
        ...
        self.assert_regression_parity(
            seed=seed, loss=loss, primitive_functions=primitive_functions,
            primitive_arities=primitive_arities, input=input,
    ...
    def assert_regression_parity(
        ...
        for i in range(n_steps):
            ...
            self.assertTrue(torch.equal(parents.dnas, parent_dnas),
                            msg=f'Wrong parents at step {i}')
            ...
            self.assertTrue(torch.equal(offspring.dnas, offspring_dnas),
            ...
            self.assertTrue(torch.equal(offspring(input), offspring_outputs),
                            msg=f'Wrong offspring eval at step {i}')
            self.assertTrue(torch.equal(offspring.fitnesses, offspring_losses),
